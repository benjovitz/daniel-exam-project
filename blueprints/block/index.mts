...
await runner(async (context: AssembleContext) => {
	...
	const name = changeCase(answers.name.trim());
	...
	await assembleTemplate({
		input: path.join(currentDir, './blueprint.block.tsx.eta'),
		output: path.join(context.cwd, `./src/blocks/${vars.name}/${vars.name}.block.tsx`),
